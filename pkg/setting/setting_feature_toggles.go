...
func (cfg *Cfg) applyFeatureToggleEnvOverrides() {
	...
	for _, env := range os.Environ() {
		...
		for _, sp := range subsectionPrefixes {
			if strings.HasPrefix(envKey, sp) {
		...
		section.Key(keyName).SetValue(envValue)
...
func (cfg *Cfg) readFeatureToggles(iniFile *ini.File) error {
	...
	if section.Key("enable").String() != "" {
		if os.Getenv(EnvKey("feature_toggles", "enable")) != "" {
			cfg.Logger.Warn("[Deprecated] The GF_FEATURE_TOGGLES_ENABLE environment variable is deprecated. Use individual GF_FEATURE_TOGGLES_<name>=true variables instead.")
		} else {
